...
var LibSQLPreparedQuery = class extends SQLitePreparedQuery {
  constructor(client2, query, logger, fields, tx, executeMethod, _isResponseInArrayMode, customResultMapper) {
...
function construct(client2, config = {}) {
  ...
  const session = new LibSQLSession(client2, dialect, schema, { logger }, void 0);
...
__export(schema_exports, {
  brisDeGlace: () => brisDeGlace,
  codeVerification: () => codeVerification,
  conseiller: () => conseiller,
  conversation: () => conversation,
  demandeConsentement: () => demandeConsentement,
  evenementAudit: () => evenementAudit,
  evenementJournal: () => evenementJournal,
  evenementQuiz: () => evenementQuiz,
  indiceConfiance: () => indiceConfiance,
  instantaneProfil: () => instantaneProfil,
  message: () => message,
  ...
  referral: () => referral,
  sessionConseiller: () => sessionConseiller,
  sessionMagicLink: () => sessionMagicLink,
  sourceCaptation: () => sourceCaptation,
  structure: () => structure,
  tiersIntervenant: () => tiersIntervenant,
  utilisateur: () => utilisateur
});
var utilisateur = sqliteTable("utilisateur", {
...
var profilRiasec = sqliteTable("profil_riasec", {
  id: text("id").primaryKey(),
...
var instantaneProfil = sqliteTable("instantane_profil", {
  id: text("id").primaryKey(),
  utilisateurId: text("utilisateur_id").notNull().references(() => utilisateur.id),
  conversationId: text("conversation_id").notNull().references(() => conversation.id),
...
var indiceConfiance = sqliteTable("indice_confiance", {
  id: text("id").primaryKey(),
  utilisateurId: text("utilisateur_id").notNull().unique().references(() => utilisateur.id),
...
var referral = sqliteTable("referral", {
  id: text("id").primaryKey(),
  utilisateurId: text("utilisateur_id").notNull().references(() => utilisateur.id),
  conversationId: text("conversation_id").notNull().references(() => conversation.id),
...
var evenementQuiz = sqliteTable("evenement_quiz", {
  id: text("id").primaryKey(),
...
var sourceCaptation = sqliteTable("source_captation", {
  id: text("id").primaryKey(),
...
var sessionMagicLink = sqliteTable("session_magic_link", {
  id: text("id").primaryKey(),
  utilisateurId: text("utilisateur_id").notNull().references(() => utilisateur.id),
...
var structure = sqliteTable("structure", {
  id: text("id").primaryKey(),
...
var conseiller = sqliteTable("conseiller", {
  id: text("id").primaryKey(),
...
var priseEnCharge = sqliteTable("prise_en_charge", {
  id: text("id").primaryKey(),
...
var sessionConseiller = sqliteTable("session_conseiller", {
  id: text("id").primaryKey(),
  conseillerId: text("conseiller_id").notNull().references(() => conseiller.id),
  jeton: text("jeton").notNull().unique(),
  expireLe: text("expire_le").notNull(),
...
var evenementAudit = sqliteTable("evenement_audit", {
  id: text("id").primaryKey(),
...
var messageDirect = sqliteTable("message_direct", {
  id: text("id").primaryKey(),
  priseEnChargeId: text("prise_en_charge_id").notNull().references(() => priseEnCharge.id),
  expediteurType: text("expediteur_type").notNull(),
  // 'beneficiaire' | 'conseiller' | 'tiers'
  expediteurId: text("expediteur_id").notNull(),
  contenu: text("contenu").notNull(),
  conversationType: text("conversation_type").default("direct"),
  // 'direct' | 'groupe' | 'tiers_beneficiaire'
  lu: integer("lu").default(0),
  horodatage: text("horodatage").notNull()
});
var tiersIntervenant = sqliteTable("tiers_intervenant", {
  id: text("id").primaryKey(),
  priseEnChargeId: text("prise_en_charge_id").notNull().references(() => priseEnCharge.id),
  nom: text("nom").notNull(),
  prenom: text("prenom").notNull(),
  ...
  role: text("role").notNull(),
  // 'employeur' | 'educateur' | 'formateur' | 'assistant_social' | 'autre'
  inviteParId: text("invite_par_id").notNull().references(() => conseiller.id),
  statut: text("statut").default("en_attente"),
  // 'en_attente' | 'approuve' | 'refuse' | 'revoque'
  creeLe: text("cree_le").notNull(),
  misAJourLe: text("mis_a_jour_le").notNull()
});
var participantConversation = sqliteTable("participant_conversation", {
  id: text("id").primaryKey(),
  priseEnChargeId: text("prise_en_charge_id").notNull().references(() => priseEnCharge.id),
  participantType: text("participant_type").notNull(),
  // 'conseiller' | 'beneficiaire' | 'tiers'
  participantId: text("participant_id").notNull(),
  actif: integer("actif").default(1),
  rejoindLe: text("rejoint_le").notNull(),
  quitteLe: text("quitte_le")
});
var demandeConsentement = sqliteTable("demande_consentement", {
  id: text("id").primaryKey(),
  priseEnChargeId: text("prise_en_charge_id").notNull().references(() => priseEnCharge.id),
  tiersId: text("tiers_id").notNull().references(() => tiersIntervenant.id),
  demandeurId: text("demandeur_id").notNull(),
  // conseiller qui a initié
  statut: text("statut").default("en_attente"),
...
var evenementJournal = sqliteTable("evenement_journal", {
  id: text("id").primaryKey(),
  priseEnChargeId: text("prise_en_charge_id").notNull().references(() => priseEnCharge.id),
  type: text("type").notNull(),
  // 'message_envoye' | 'participant_rejoint' | 'participant_quitte' | 'consentement_demande' | 'consentement_accepte' | 'consentement_refuse' | 'video_proposee' | 'video_acceptee' | 'video_refusee' | 'rdv_planifie' | 'bris_de_glace' | 'tiers_invite' | 'tiers_revoque'
  acteurType: text("acteur_type").notNull(),
  // 'conseiller' | 'beneficiaire' | 'tiers' | 'systeme'
  acteurId: text("acteur_id").notNull(),
  cibleType: text("cible_type"),
  // entité ciblée (optionnel)
  cibleId: text("cible_id"),
  resume: text("resume"),
  // résumé lisible en 1 ligne
  details: text("details"),
  // JSON avec métadonnées supplémentaires
  horodatage: text("horodatage").notNull()
});
var brisDeGlace = sqliteTable("bris_de_glace", {
  id: text("id").primaryKey(),
  conseillerId: text("conseiller_id").notNull().references(() => conseiller.id),
  priseEnChargeId: text("prise_en_charge_id").notNull().references(() => priseEnCharge.id),
  tiersId: text("tiers_id").notNull().references(() => tiersIntervenant.id),
...
var codeVerification = sqliteTable("code_verification", {
  id: text("id").primaryKey(),
  referralId: text("referral_id").notNull().references(() => referral.id),
...
// scripts/seed-extended.ts
var client = (0, import_client2.createClient)({ url: process.env.TURSO_DATABASE_URL || "file:local.db" });
var db = drizzle(client, { schema: schema_exports });
var hash = (pw) => import_bcryptjs.default.hashSync(pw, 12);
var now = () => (/* @__PURE__ */ new Date()).toISOString();
var ago = (hours) => new Date(Date.now() - hours * 36e5).toISOString();
var PASSWORD = hash("catchup2026");
var NEW_STRUCTURES = [
...
var NEW_CONSEILLERS = [
  ...
  { email: "marine.kerouac@cio-rennes.fr", prenom: "Marine", nom: "K\xE9rouac", role: "conseiller", structIdx: 1 },
  ...
  { email: "elise.robert@ml-nantes.fr", prenom: "\xC9lise", nom: "Robert", role: "conseiller", structIdx: -1 }
];
var PRENOMS_M = ["Enzo", "Noah", "Mehdi", "Yanis", "Adam", "Hugo", "Nathan", "Th\xE9o", "Amine", "Ilyes", "Dylan", "Kylian", "Mathis", "Axel", "Sacha", "Ibrahim", "Tom", "Nolan", "Rayan", "Gabin"];
var PRENOMS_F = ["Jade", "L\xE9na", "Chlo\xE9", "Inaya", "Manon", "Clara", "Zo\xE9", "Camille", "Sarah", "Amira", "Lina", "Ma\xEBlle", "No\xE9mie", "Alicia", "Yasmine", "Margot", "Juliette", "Oc\xE9ane", "Eva", "Nina"];
var SITUATIONS = ["lyceen", "decrocheur", "etudiant", "demandeur_emploi", "alternant", "neet", "service_civique"];
var DEPS = ["75", "92", "93", "94", "69", "13", "59", "33", "31", "44", "67", "35", "38", "06", "34", "76", "57", "45", "21", "29"];
var PHASES = ["accroche", "decouverte", "exploration", "projection", "action"];
var PRIORITES = ["normale", "normale", "normale", "haute", "haute", "critique"];
var MOTIFS = [
  "D\xE9crochage scolaire, recherche d'orientation",
  "H\xE9sitation post-bac, besoin de conseil Parcoursup",
  "Recherche d'alternance, pas de r\xE9seau",
  "Difficult\xE9s familiales impactant le parcours scolaire",
  "R\xE9orientation apr\xE8s \xE9chec en L1",
  "NEET depuis 6 mois, isolement social",
  "Besoin d'aide pour les d\xE9marches administratives",
  "Stress intense li\xE9 \xE0 l'orientation",
  "D\xE9couverte des m\xE9tiers, aucune id\xE9e de carri\xE8re",
  "Situation de handicap, besoin d'accompagnement sp\xE9cialis\xE9",
  "Envie d'entreprendre mais pas de soutien",
  "Conflit avec les parents sur le choix d'\xE9tudes",
  "Pr\xE9carit\xE9 financi\xE8re, recherche de solutions",
  "Harc\xE8lement scolaire, changement d'\xE9tablissement",
  "Expatriation r\xE9cente, int\xE9gration difficile"
];
var RESUMES = [
  (p, a) => `${p}, ${a} ans, en situation de d\xE9crochage scolaire. A exprim\xE9 un int\xE9r\xEAt pour les m\xE9tiers manuels et techniques. Conversation constructive, le jeune est motiv\xE9 mais manque de rep\xE8res.`,
  (p, a) => `${p}, ${a} ans, lyc\xE9en(ne) en questionnement sur son orientation post-bac. Profil bien d\xE9fini, h\xE9site entre plusieurs fili\xE8res. A besoin d'aide pour prioriser ses choix.`,
  (p, a) => `${p}, ${a} ans, en recherche d'emploi depuis plusieurs mois. A \xE9voqu\xE9 des difficult\xE9s financi\xE8res et un sentiment d'isolement. Souhaite un accompagnement rapide.`,
  (p, a) => `${p}, ${a} ans, souhaite se r\xE9orienter apr\xE8s une premi\xE8re ann\xE9e universitaire difficile. Profil atypique, int\xE9ress\xE9(e) par des voies alternatives. Conversation riche et ouverte.`,
  (p, a) => `${p}, ${a} ans, jeune en grande fragilit\xE9 \xE9motionnelle. A \xE9voqu\xE9 des tensions familiales et un mal-\xEAtre profond. Orientation vers un accompagnement social prioritaire.`
];
var CONVERSATIONS_TEMPLATES = [
  // Template court (4 messages)
  [
    { role: "assistant", contenu: "Salut ! Moi c'est Catch'Up, je suis l\xE0 pour t'aider \xE0 trouver ta voie. Qu'est-ce qui t'am\xE8ne ici ?" },
    { role: "user", contenu: "Salut, je sais pas trop quoi faire apr\xE8s le lyc\xE9e, je suis un peu perdu" },
    { role: "assistant", contenu: "C'est normal de se poser des questions \xE0 ce stade ! Dis-moi, qu'est-ce que tu aimes faire quand tu as du temps libre ?" },
    { role: "user", contenu: "J'aime bien le sport et les jeux vid\xE9o, et j'aide souvent mes potes quand ils ont des probl\xE8mes" }
  ],
  // Template moyen (6 messages)
  [
    { role: "assistant", contenu: "Hey ! Bienvenue sur Catch'Up \u{1F680} Je suis l\xE0 pour discuter avec toi de ton avenir. Comment \xE7a va ?" },
    { role: "user", contenu: "Bof, pas super. J'ai arr\xEAt\xE9 les cours et mes parents sont sur mon dos" },
    { role: "assistant", contenu: "Je comprends, c'est une p\xE9riode compliqu\xE9e. Tu as arr\xEAt\xE9 r\xE9cemment ? Tu \xE9tais dans quel type de formation ?" },
    { role: "user", contenu: "J'\xE9tais en BEP mais \xE7a me plaisait pas du tout. Trop de th\xE9orie, pas assez de pratique" },
    { role: "assistant", contenu: "D'accord, tu es quelqu'un qui pr\xE9f\xE8re apprendre en faisant. C'est une vraie qualit\xE9 ! Qu'est-ce qui te branche comme activit\xE9s ?" },
    { role: "user", contenu: "La cuisine, j'adore \xE7a. Je cuisine tous les jours chez moi. Et j'aimerais bien avoir mon resto un jour" }
  ],
  // Template long (10 messages)
  [
    { role: "assistant", contenu: "Salut ! Je suis Catch'Up, ton assistant orientation. On discute un peu ? \u{1F60A}" },
    { role: "user", contenu: "Ok, je veux bien. On m'a dit que \xE7a pouvait m'aider" },
    { role: "assistant", contenu: "Super ! Pour commencer, tu es en quelle situation en ce moment ? Tu es au lyc\xE9e, en recherche d'emploi... ?" },
    { role: "user", contenu: "Je suis en terminale mais j'ai aucune id\xE9e de ce que je veux faire. Tout le monde me met la pression" },
    { role: "assistant", contenu: "La pression, \xE7a peut \xEAtre difficile \xE0 g\xE9rer. Tu n'es pas oblig\xE9(e) d'avoir toutes les r\xE9ponses maintenant. Parlons plut\xF4t de ce qui te fait vibrer dans la vie ?" },
    { role: "user", contenu: "J'aime aider les gens. Genre quand quelqu'un va pas bien, je suis toujours l\xE0. Et j'aime aussi les sciences" },
    { role: "assistant", contenu: "C'est super int\xE9ressant ! L'envie d'aider les autres combin\xE9e aux sciences, \xE7a ouvre plein de possibilit\xE9s. Tu as d\xE9j\xE0 pens\xE9 au domaine m\xE9dical ou param\xE9dical ?" },
    { role: "user", contenu: "Oui un peu, mais mes notes sont pas terribles en maths. Du coup je me dis que c'est pas pour moi" },
    { role: "assistant", contenu: "Il y a beaucoup de m\xE9tiers dans le soin et l'accompagnement qui ne demandent pas forc\xE9ment un niveau \xE9lev\xE9 en maths. Infirmier, \xE9ducateur sp\xE9cialis\xE9, aide-soignant... Tu veux qu'on en parle ?" },
    { role: "user", contenu: "Oui carr\xE9ment ! \xC9ducateur sp\xE9cialis\xE9 \xE7a me parle bien en fait" }
  ],
  ...
  [
    ...
    { role: "assistant", contenu: "Je suis d\xE9sol\xE9 que tu traverses un moment difficile. Tu veux bien m'en dire un peu plus sur ce qui ne va pas ?" },
    { role: "user", contenu: "Mes parents veulent que je fasse m\xE9decine mais moi j'aime le dessin. Ils comprennent rien" },
    ...
    { role: "user", contenu: "Oui c'est la seule chose qui me fait du bien. Mais j'ai l'impression que \xE7a sert \xE0 rien" },
    { role: "assistant", contenu: "Le dessin, c'est loin d'\xEAtre inutile. Il y a plein de m\xE9tiers cr\xE9atifs qui recrutent : graphisme, illustration, game design, architecture... Et tes sentiments sont valides. Si tu le souhaites, un conseiller pourrait t'aider \xE0 construire un projet qui te ressemble." },
    { role: "user", contenu: "Oui je veux bien parler \xE0 quelqu'un qui comprend" }
  ]
];
var RIASEC_PROFILES = [
  { r: 80, i: 30, a: 20, s: 40, e: 55, c: 25, dom: ["R", "E"], traits: ["concret", "d\xE9brouillard"], interets: ["bricolage", "m\xE9canique"], forces: ["manuel", "endurant"] },
  { r: 25, i: 70, a: 40, s: 50, e: 30, c: 55, dom: ["I", "C"], traits: ["analytique", "curieux"], interets: ["sciences", "informatique"], forces: ["logique", "rigueur"] },
  { r: 15, i: 30, a: 85, s: 60, e: 25, c: 20, dom: ["A", "S"], traits: ["cr\xE9atif", "sensible"], interets: ["dessin", "musique", "\xE9criture"], forces: ["imagination", "empathie"] },
  { r: 30, i: 25, a: 40, s: 80, e: 45, c: 35, dom: ["S", "E"], traits: ["empathique", "altruiste"], interets: ["aide aux autres", "\xE9ducation"], forces: ["\xE9coute", "communication"] },
  { r: 45, i: 35, a: 25, s: 30, e: 75, c: 40, dom: ["E", "R"], traits: ["ambitieux", "leader"], interets: ["commerce", "entrepreneuriat"], forces: ["charisme", "pers\xE9v\xE9rance"] },
  { r: 20, i: 45, a: 30, s: 35, e: 40, c: 80, dom: ["C", "I"], traits: ["organis\xE9", "m\xE9thodique"], interets: ["comptabilit\xE9", "administration"], forces: ["pr\xE9cision", "fiabilit\xE9"] },
  { r: 60, i: 50, a: 35, s: 55, e: 30, c: 40, dom: ["R", "S"], traits: ["patient", "observateur"], interets: ["nature", "animaux", "sant\xE9"], forces: ["dext\xE9rit\xE9", "pers\xE9v\xE9rance"] },
  { r: 35, i: 65, a: 70, s: 40, e: 50, c: 25, dom: ["A", "I"], traits: ["innovant", "ind\xE9pendant"], interets: ["technologie", "design", "musique"], forces: ["cr\xE9ativit\xE9", "autodidacte"] },
  { r: 50, i: 40, a: 30, s: 65, e: 70, c: 35, dom: ["E", "S"], traits: ["sociable", "dynamique"], interets: ["sport", "animation", "\xE9v\xE9nementiel"], forces: ["\xE9nergie", "organisation"] },
  { r: 40, i: 75, a: 45, s: 30, e: 35, c: 60, dom: ["I", "C"], traits: ["perfectionniste", "r\xE9fl\xE9chi"], interets: ["math\xE9matiques", "programmation"], forces: ["concentration", "analyse"] }
];
async function seed() {
  console.log("\u{1F331} Seed \xE9tendu \u2014 Ajout de donn\xE9es suppl\xE9mentaires...\n");
  const t = now();
  const structureIds = [];
  for (const s of NEW_STRUCTURES) {
    ...
    try {
      ...
      if (existing.rows.length > 0) {
        structureIds.push(existing.rows[0].id);
  ...
  for (const c of NEW_CONSEILLERS) {
    ...
    try {
      ...
      if (existing.rows.length > 0) {
        conseillerIds.push(existing.rows[0].id);
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const hoursAgo = 2 + Math.floor(Math.random() * 120);
    ...
    await db.insert(indiceConfiance).values({
      id: v4_default(),
      utilisateurId: userId,
    ...
    await db.insert(conversation).values({
      ...
      phase,
      dureeSecondes: convTemplate.length * 45 + Math.floor(Math.random() * 300),
      creeLe: ago(hoursAgo),
      misAJourLe: ago(hoursAgo - 0.5)
    });
    for (let m = 0; m < convTemplate.length; m++) {
      ...
      await db.insert(message).values({
        ...
        role: msg.role,
        contenu: msg.contenu,
        contenuBrut: msg.contenu,
        fragiliteDetectee: "fragilite" in msg && msg.fragilite ? 1 : 0,
        niveauFragilite: "niveauFragilite" in msg ? msg.niveauFragilite : null,
        horodatage: msgTime
      });
    }
    const referralId = v4_default();
    ...
    await db.insert(referral).values({
      ...
      conversationId: convId,
      priorite,
      niveauDetection: priorite === "critique" ? 3 : priorite === "haute" ? 2 : 1,
      motif,
      resumeConversation: resumeFn(prenom, age),
      moyenContact: `06${String(1e7 + Math.floor(Math.random() * 89999999))}`,
    ...
    if (i < 12) {
      ...
      await db.insert(priseEnCharge).values({
        id: v4_default(),
        referralId,
        conseillerId: cons.id,
        structureId: cons.structureId,
        statut: i < 8 ? "prise_en_charge" : "terminee",
        premiereActionLe: ago(hoursAgo - 2),
        creeLe: ago(hoursAgo - 2),
        misAJourLe: ago(hoursAgo - 3)
      });
    }
    const statusLabel = i < 8 ? "\u{1F91D}" : i < 12 ? "\u2705" : "\u23F3";
    console.log(`  ${statusLabel} ${prenom} (${age} ans, ${dep}) \u2014 ${priorite} \u2014 ${phase}`);
  }
  console.log("\n\u2705 Seed \xE9tendu termin\xE9 !");
  console.log("   +3 structures, +9 conseillers, +40 b\xE9n\xE9ficiaires\n");
  console.log("   Totaux estim\xE9s : 10 structures, 20 conseillers, ~63 b\xE9n\xE9ficiaires\n");
  process.exit(0);
}
seed().catch((e) => {
