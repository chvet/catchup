...
// CRUD tarifications pour structures privées
...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    // Vérifier que la structure existe et est privée
    ...
    if (structures[0].visibilite !== 'privee') {
      return jsonError('La tarification n\'est disponible que pour les structures privees', 400)
    }

    const body = await request.json()
    const { libelle, description, montantCentimes, devise, dureeJours } = body

    if (!libelle || typeof libelle !== 'string' || libelle.trim().length === 0) {
      return jsonError('Le libelle est requis', 400)
    }
    if (!montantCentimes || typeof montantCentimes !== 'number' || montantCentimes <= 0) {
      return jsonError('Le montant doit etre un nombre positif (en centimes)', 400)
    }
